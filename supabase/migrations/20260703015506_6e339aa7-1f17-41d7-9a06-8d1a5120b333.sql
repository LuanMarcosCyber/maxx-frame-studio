...
BEGIN
  ...
  SELECT parent_user_id INTO v_target_parent
  ...
  -- Conta de acesso herda sempre da conta pai direta.
  -- Admin/Revendedor sem pai usa os próprios dados.
  v_store_id := COALESCE(v_target_parent, _user_id);

  -- Autoriza o próprio usuário, a conta pai direta, ancestrais/descendentes da mesma hierarquia ou admin.
  IF v_caller = _user_id OR v_caller = v_store_id OR public.has_role(v_caller, 'admin') THEN
    v_authorized := true;
  ELSE
    -- Caller sobe a árvore: pode acessar se encontrar o alvo ou a loja herdada.
    ...
    -- Target sobe a árvore: pode acessar se encontrar o caller.
